...
describe('FlowOps Dashboard App Component', () => {
  ...
  describe('Pipeline Settings Form Integration Tests', () => {
    it('opens and closes the settings modal', () => {
      ...
      const settingsBtn = screen.getByRole('button', { name: /Pipeline Settings/i });
      fireEvent.click(settingsBtn);
      
      // Modal should now be open
      expect(screen.getByTestId('settings-modal')).toBeInTheDocument();
      expect(screen.getByText('Pipeline Settings Configuration')).toBeInTheDocument();
    ...
    it('renders default form values and handles input changes', () => {
      ...
      const webhookInput = screen.getByLabelText(/Slack Notification Webhook URL/i);
      ...
      fireEvent.change(webhookInput, { target: { value: 'https://hooks.slack.com/services/123' } });
    ...
    it('displays validation errors for invalid timeout values', () => {
      ...
      // Open settings
      fireEvent.click(screen.getByRole('button', { name: /Pipeline Settings/i }));
      ...
      expect(screen.getByText('Timeout must be between 1 and 120 minutes')).toBeInTheDocument();
    ...
    it('displays validation errors for invalid Slack Webhook format', () => {
      ...
      const webhookInput = screen.getByLabelText(/Slack Notification Webhook URL/i);
      fireEvent.change(webhookInput, { target: { value: 'https://invalid-url.com/webhook' } });
      
      // Click submit
      fireEvent.click(screen.getByRole('button', { name: /Save Changes/i }));
      
      // Verify validation error
      expect(screen.getByText('Webhook must start with https://hooks.slack.com/')).toBeInTheDocument();
    ...
    it('submits successfully and updates active configuration panel on the dashboard', () => {
      ...
      // Modify values
      fireEvent.change(screen.getByLabelText(/Target Environment/i), { target: { value: 'development' } });
      fireEvent.change(screen.getByLabelText(/Build Timeout/i), { target: { value: '15' } });
      fireEvent.click(screen.getByLabelText(/Run Security Scan/i)); // Disable security scan
      fireEvent.change(screen.getByLabelText(/Slack Notification Webhook URL/i), {
      ...
      expect(screen.getByTestId('settings-env').textContent).toBe('development');
      expect(screen.getByTestId('settings-timeout').textContent).toBe('15 mins');
      expect(screen.getByTestId('settings-security').textContent).toBe('Disabled');
      expect(screen.getByTestId('settings-docker').textContent).toBe('Enabled'); // Unchanged
      expect(screen.getByTestId('settings-webhook').textContent).toBe('https://hooks.slack.com/services/ABC');
    });
  });
  ...
  describe('Secrets & Variables Manager Page Tests', () => {
    it('navigates to Secrets Manager tab and renders the initial list and form', () => {
      ...
      expect(screen.queryByText('Active Environment Variables & Secrets')).not.toBeInTheDocument();
      
      // Click "Secrets Manager" tab button
      const secretsTabBtn = screen.getAllByRole('button', { name: /Secrets Manager/i })[0];
      fireEvent.click(secretsTabBtn);
      
      // Verification
      expect(screen.getByText('Active Environment Variables & Secrets')).toBeInTheDocument();
    ...
    it('verifies default secrets are masked and toggle show/hide reveal works', () => {
      ...
      // Open page
      fireEvent.click(screen.getAllByRole('button', { name: /Secrets Manager/i })[0]);
      
      // DOCKER_USERNAME is plain (revealed) by default
      expect(screen.getByTestId('secret-value-DOCKER_USERNAME').textContent).toBe('vatsalpalm');
      expect(screen.queryByTestId('reveal-btn-DOCKER_USERNAME')).not.toBeInTheDocument(); // no reveal button for plain type
      ...
      expect(screen.getByTestId('secret-value-SONAR_TOKEN').textContent).toBe('sqa_987654321abcdef0123456789');
    ...
    it('validates invalid variable name format', () => {
      ...
      expect(screen.getByText('Variable Name must only contain uppercase letters, numbers, and underscores, and start with a letter')).toBeInTheDocument();
    ...
    it('validates empty inputs validation checks', () => {
      ...
      expect(screen.getByText('Variable value is required')).toBeInTheDocument();
    });
    ...
    it('adds new variable/secret successfully and clears form inputs', () => {
      ...
      fireEvent.change(valueInput, { target: { value: 'xoxb-slack-secret-token-key' } });
      
      // Radio buttons are already checked as "secret" by default, scope All Environments by default.
      fireEvent.click(submitBtn);
      
      // Success alert
      expect(screen.getByText('Variable added successfully!')).toBeInTheDocument();
      ...
      fireEvent.click(revealBtn);
      expect(screen.getByTestId('secret-value-SLACK_TOKEN').textContent).toBe('xoxb-slack-secret-token-key');
    });
