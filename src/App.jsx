...
const INITIAL_SONAR_REPORT = {
  ...
  issues: [
    { id: 'SQ-101', severity: 'minor', type: 'Code Smell', message: 'Remove unused import "reactLogo" in App.jsx', file: 'src/App.jsx', line: 2 },
    { id: 'SQ-102', severity: 'minor', type: 'Code Smell', message: 'Remove unused import "viteLogo" in App.jsx', file: 'src/App.jsx', line: 3 },
    { id: 'SQ-103', severity: 'major', type: 'Code Smell', message: 'Define static constants outside component function scope', file: 'src/App.jsx', line: 8 },
    { id: 'SQ-104', severity: 'minor', type: 'Code Smell', message: 'Add alt text descriptive tag to SVG icons for accessibility', file: 'src/App.jsx', line: 95 }
  ]
}
...
function App() {
  ...
  const [pipelineLogs, setPipelineLogs] = useState(STAGE_DETAILS[selectedStage].logs)
    ...
    <div className="dashboard-container">
      ...
      <main className="main-content">
        ...
        <section className="hero-section">
          ...
          <h1 className="hero-title">Your Development Automation Center</h1>
          <p className="hero-subtitle">
            Configure integration actions, view live build status flow charts, and analyze deep code-quality metrics powered by SonarQube scanner.
          </p>
        </section>
          ...
          >
            ...
            {sonarReport.issues.length > 0 && (
              <span style={{
                ...
                padding: '1px 6px',
                marginLeft: '4px'
              }}>{sonarReport.issues.length}</span>
            )}
          </button>
        </div>
            ...
            <section className="glass-card pipeline-visual-card">
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <div style={{ display: 'flex', gap: '0.75rem' }}>
                  ...
                  >
                    {isPipelineTriggered ? 'Triggering...' : 'Trigger Pipeline Run'}
                  </button>
        ...
        {activeTab === 'sonar' && (
          <section className="sonar-container">
            ...
            <div className={`sonar-status-banner ${isSonarScanning ? 'scanning' : sonarReport.status === 'passed' ? 'passed' : 'failed'}`}>
              <div>
                <h2 className="sonar-status-title">
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" style={{ color: isSonarScanning ? 'var(--color-cyan)' : 'var(--color-emerald)' }}>
                    <path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" />
                  </svg>
                  {isSonarScanning ? 'SonarQube Analysis in Progress...' : 'Quality Gate Status'}
                </h2>
                <p className="metric-desc" style={{ marginTop: '0.5rem', color: 'var(--text-secondary)' }}>
                  {isSonarScanning ? `Executing phase: ${sonarScanPhase}` : `Project scanned ${sonarReport.lastScan} on main branch.`}
                </p>
              </div>
              <div>
                {isSonarScanning ? (
                  ...
                  <span className={`sonar-gate-badge ${sonarReport.status === 'passed' ? 'passed' : 'failed'}`}>
            ...
            <div className="sonar-metrics-grid">
              ...
              <div className="glass-card" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', minHeight: '140px' }}>
                ...
                <div>
                  <div className="metric-value">{isSonarScanning ? '--' : sonarReport.metrics.bugs.value}</div>
                  <span className="metric-desc" style={{ color: 'var(--color-emerald)' }}>{isSonarScanning ? 'Calculating...' : 'Reliability rating'}</span>
                </div>
              </div>
              ...
              <div className="glass-card" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', minHeight: '140px' }}>
                ...
                <div>
                  <div className="metric-value">{isSonarScanning ? '--' : sonarReport.metrics.vulnerabilities.value}</div>
                  <span className="metric-desc" style={{ color: 'var(--color-emerald)' }}>{isSonarScanning ? 'Calculating...' : 'Security rating'}</span>
                </div>
              </div>
              ...
              <div className="glass-card" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', minHeight: '140px' }}>
                ...
                <div>
                  <div className="metric-value">{isSonarScanning ? '--' : sonarReport.metrics.securityHotspots.value}</div>
                  <span className="metric-desc">{isSonarScanning ? 'Reviewing...' : sonarReport.metrics.securityHotspots.status}</span>
                </div>
              </div>
              ...
              <div className="glass-card" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', minHeight: '140px' }}>
                ...
                <div>
                  <div className="metric-value" style={{ color: !isSonarScanning && sonarReport.metrics.codeSmells.value > 0 ? 'var(--color-amber)' : 'inherit' }}>
                    {isSonarScanning ? '--' : sonarReport.metrics.codeSmells.value}
                  </div>
                  <span className="metric-desc">{isSonarScanning ? 'Checking debt...' : sonarReport.metrics.codeSmells.status}</span>
                </div>
              </div>
              ...
              <div className="glass-card" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', minHeight: '140px' }}>
                ...
                <div>
                  <div className="metric-value" style={{ color: 'var(--color-emerald)' }}>{isSonarScanning ? '--' : sonarReport.metrics.coverage.value}</div>
                  <span className="metric-desc">{isSonarScanning ? 'Compiling tests...' : 'Unit test pathways'}</span>
                </div>
              </div>
              ...
              <div className="glass-card" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', minHeight: '140px' }}>
                ...
                <div>
                  <div className="metric-value">{isSonarScanning ? '--' : sonarReport.metrics.duplications.value}</div>
                  <span className="metric-desc">{isSonarScanning ? 'Checking clones...' : 'Zero duplicated blocks'}</span>
                </div>
              </div>
            </div>

            {/* Run scanner & Logs block */}
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', alignItems: 'start' }} className="responsive-split">
              
              {/* Scan controller block */}
              <div className="glass-card" style={{ height: '100%' }}>
                <h3 className="font-display" style={{ fontSize: '1.125rem', fontWeight: '700', marginBottom: '0.75rem' }}>Trigger Quality Scan</h3>
                <p className="metric-desc" style={{ marginBottom: '1.5rem' }}>
                  Pushing code to GitHub triggers the pipeline scan automatically. You can also trigger a manual scan analysis of your local directories here.
                </p>
                <div style={{ display: 'flex', gap: '0.75rem' }}>
                  <button 
                  ...
                  >
                    {isSonarScanning ? 'Scanning Code...' : 'Trigger Quality Scan'}
                  </button>
                  <button 
                    onClick={() => {
              ...
              <div style={{
                ...
                <h4 className="font-mono" style={{ fontSize: '0.875rem', borderBottom: '1px solid rgba(255, 255, 255, 0.05)', paddingBottom: '0.5rem', marginBottom: '1rem', color: 'var(--text-secondary)' }}>
                  Scanner Logs
                </h4>
                <div className="font-mono" style={{ fontSize: '0.75rem', lineHeight: '1.8', color: '#a1a7b5', maxHeight: '140px', overflowY: 'auto' }}>
                  {sonarScanLogs.length === 0 ? (
                    <span style={{ color: 'var(--text-muted)' }}>Idle. Trigger scan to review analyzer execution logs.</span>
                  ) : (
                  ...
                  {isSonarScanning && (
                    <div style={{ color: 'var(--color-cyan)', display: 'flex', alignItems: 'center', gap: '4px', marginTop: '0.25rem' }}>
                      <span style={{ animation: 'sonar-pulse 1.2s infinite ease-in-out', display: 'inline-block' }}>●</span> 
            ...
            <div className="glass-card sonar-issues-card">
              <h3 className="font-display" style={{ fontSize: '1.125rem', fontWeight: '700' }}>Active Code Quality Issues</h3>
              <p className="metric-desc" style={{ marginTop: '0.25rem' }}>
                Fixing these issues in your codebase and pushing again will clear the quality gate.
              </p>
              ...
              {sonarReport.issues.length === 0 ? (
                <div style={{
                  ...
                  <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" style={{ background: 'rgba(16, 185, 129, 0.1)', padding: '8px', borderRadius: '50%' }}>
                    <polyline points="20 6 9 17 4 12" />
                  </svg>
                  <span>Excellent! No code quality issues found in active codebase.</span>
                </div>
              ) : (
                  ...
                  <table className="sonar-issues-table">
                    ...
                    <tbody>
                      {sonarReport.issues.map((issue) => (
                        <tr key={issue.id}>
                          <td className="font-mono" style={{ color: 'var(--color-indigo)', fontSize: '0.8125rem', fontWeight: '600' }}>{issue.id}</td>
                          <td>
      ...
      <footer className="footer">
        <p className="footer-text">© {new Date().getFullYear()} FlowOps Pipeline Orchestrator. Configured with GitHub Actions & SonarQube.</p>
      </footer>
    </div>
  )
}
